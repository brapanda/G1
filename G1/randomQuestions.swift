...
class randomQuestion: UIViewController,UIActionSheetDelegate,UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UIAlertViewDelegate {
    var managedContext: NSManagedObjectContext! = RQCoreData().context
    var beginPage = 0
    ...
    override func viewDidLoad() {
        ...
        if let selections: AnyObject = UserData.valueForKey("selectedList_random"){
            selectedList = selections as! NSDictionary as! [String: Int]
        }else{
            UserData.setValue(selectedList, forKey: "selectedList_random")
        }
        if let pages: AnyObject = UserData.valueForKey("pageList_random") {
            pageList = pages as! NSDictionary as! [String: Int]
        }else{
            UserData.setValue(pageList, forKey: "pageList_random")
        }
        totalCorrectAns = UserData.integerForKey("totalCorrectAns_random") as NSInteger
        ...
        let loadBool = UserData.boolForKey("loadData_random")
        if loadBool == false{
            ...
            loading.animateToAngle(360, duration: 1.5) { completed in
                ...
                if newPage < self.QuestionsSet.count{
                    self.createCard(self.QuestionsSet[newPage] as! RQuestion, page: newPage)
                    self.storyScrollView.loadPageIndex(UInt(newPage), animated: false)
                }else{
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        if loadBool == false{
            loadData()
            setData()
            UserData.setBool(true, forKey: "loadData_random")
        }else{
            setData()
        }
        ...
        if QuestionsSet.count > 1{
            let q = QuestionsSet[0] as! RQuestion
            createCard(q,page: 0)
            let q2 = QuestionsSet[1] as! RQuestion
        ...
        let loadBools = UserData.boolForKey("loadData_random")
        if loadBools == true{
            self.loading.removeFromSuperview()
            self.view.addSubview(self.storyScrollView)
            self.view.addSubview(self.overView)
    ...
    func loadData(){
        ...
        let questionFetch = NSFetchRequest(entityName: "RQuestion")
        ...
        for i in 1...keysList.count {
            let entity = NSEntityDescription.entityForName("RQuestion", inManagedObjectContext: managedContext)
            let ques = RQuestion(entity: entity!, insertIntoManagedObjectContext: managedContext)
            let randomNum = arc4random_uniform(UInt32(keysList.count))
            let key = keysList[Int(randomNum)] as! String
            ...
            //println(dataDictionary?.valueForKey(key))
            ...
            keysList.removeAtIndex(Int(randomNum))
        }
    ...
    func setData(){
        ...
        let entity = NSEntityDescription.entityForName("RQuestion", inManagedObjectContext: managedContext)
        let question = RQuestion(entity: entity!, insertIntoManagedObjectContext: managedContext)
        let tagFetch = NSFetchRequest(entityName: "RQuestion")
        tagFetch.predicate = NSPredicate(format: "tag != 0")
        var error: NSError?
        
        let results = managedContext.executeFetchRequest(tagFetch, error: &error) as! [RQuestion]
        let loadBool = UserData.boolForKey("loadData_random")
        if loadBool == false{
            for result in results {
                let ques = result as! RQuestion
    ...
    func resetData(){
        ...
        UserData.setValue([String: Int](), forKey: "selectedList_random")
        UserData.setBool(false, forKey: "loadData_random")
        ...
        UserData.setValue(selectedList, forKey: "selectedList_random")
        UserData.setValue(pageList, forKey: "pageList_random")
        UserData.setInteger(0, forKey: "totalCorrectAns_random")
        totalCorrectAns = 0
        totalCorrect.text = "正确: " + toString(totalCorrectAns) + " of " + toString(totalQuestion)
    ...
    func backToMain(){
        UserData.setValue(totalCorrectAns, forKey: "totalCorrectAns_random")
    ...
    func createCard(questionData: RQuestion,page: Int){
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        if (scrollView.isKindOfClass(UIScrollView)) {
            ...
            if (storyScrollView.contentOffset.x > storyScrollView.frame.size.width * CGFloat(currentPage)) && (QuestionsSet.count > totalPage){
                let q = QuestionsSet[totalPage] as! RQuestion
            ...
            if lastPage > 0 && (storyScrollView.contentOffset.x < CGFloat(currentPage) * storyScrollView.frame.size.width){
                if !contains(alreadyLoad,lastPage){
                    let q = QuestionsSet[lastPage] as! RQuestion
        ...
        for i in QuestionsSet{
            ...
            if q.tag == tableView.tag{
                ...
                if q.correct == indexPath.row {
                    ...
                    UserData.setInteger(totalCorrectAns, forKey: "totalCorrectAns_random")
                }else{
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        */
        ...
        if let tag = pageList[toString(indexPath.row)] {
            let q = QuestionsSet[indexPath.row] as! RQuestion
    ...
    func savePage(page: Int){
        UserData.setInteger(page, forKey: "regularPage_random")
    }
    
    func getPage() -> Int{
        let newPage = UserData.integerForKey("regularPage_random")
        return newPage
    }
